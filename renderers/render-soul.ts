...
function renderSoul({ imageContext, transform }, soul: Soul) {
  // Is save expensive?
  imageContext.save();

  const x = transform.applyX(soul.x) - soul.sprite.width / 2;
  const y = transform.applyY(soul.y) - soul.sprite.height / 2;
  var localX = 0;
  var localY = 0;
  imageContext.translate(x, y);
  // Sprites are assumed to face the right by default.
  if (isEqual(soul.facing, [-1, 0])) {
    ...
    const angle: number = Math.atan2(soul.facing[1], soul.facing[0]);
